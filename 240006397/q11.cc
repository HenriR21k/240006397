...
void print_dep() {
  ...
  vector<pair<int, string*>> department_vector;
  for (const auto& entry : department) {
    department_vector.push_back(entry);
  }
  sort(department_vector.begin(), department_vector.end(), 
       [](const pair<int, string*>& a, const pair<int, string*>& b) {
         return *(a.second) < *(b.second);
       });

  for (const auto& entry : department_vector) {
...
void print_inner_join() {
  ...
  for (const auto& entry : employee) {
    string* department_name = department[entry.second];
    if (department_name != nullptr) {
      cout << get_null_or_X(entry.first)
           << get_null_or_X(entry.second)
           << get_null_or_X(department_name->c_str())
           << endl;
    }
  }
}
